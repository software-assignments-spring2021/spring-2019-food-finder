...
app.get("/test", function(request, response) {
  ...
  if (
    query.zipcode == "" &&
    (query.cuisine_type == "" || query.cuisine_type == "No Preference") &&
    (query.borough == "" || query.borough == "No Preference")
  ) {
  ...
  //This is if there's no zipcode, no preference/nothing for cuisine type, a borough specified
    ...
    query.zipcode == "" &&
    (query.cuisine_type == "" || query.cuisine_type == "No Preference") &&
    (query.borough != "No Preference" && query.borough != "Nearby")
  ) {
    const newQuery = {
      borough: request.query.walkingTime
    };
    db.collection("restaurantData")
      .aggregate([{ $match: newQuery }, { $sample: { size: 5 } }])
      .toArray(function(err, docs) {
        if (err) {
          console.log("err");
        } else {
  ...
  User.findOne({ username: userObject.username }, (error, user) => {
    if (error) {
      ...
      console.log(
        "User with username " +
          userObject.username +
          " already exists inside the collection!"
      );
    } else {
      // new user! this should add the new user to collection
